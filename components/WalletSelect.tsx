  ...
  return (
    <Menu>
      {({ open }) => (
        <>
          <Menu.Button
            className={`solape__wallet-select-btn h-full w-8 cursor-pointer items-center justify-center rounded-none rounded-r-lg bg-th-primary-dark text-th-bkg-1 hover:brightness-[1.1] focus:outline-1`}
          >
            <ChevronDownIcon
              className={`default-transition h-5 w-7 ${open ? 'rotate-180 transform' : 'rotate-360 transform'
              }`}
            />
          </Menu.Button>
          <Transition
