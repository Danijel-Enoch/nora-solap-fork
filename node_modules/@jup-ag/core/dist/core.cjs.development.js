...
async function chunkedGetMultipleAccountInfos(connection, pks, chunkSize = 100) {
  return (await Promise.all(chunks(pks, chunkSize).map(chunk => {
    ...
    return (// getMultipleAccounts is quite slow, so we use fetch directly
      ...
      ._rpcRequest('getMultipleAccounts', args).then(item => {
...
class SaberAmm {
  constructor(stableSwap) {
    ...
    this.calculator = new math.Stable(math.TWO, JSBI__default["default"].BigInt(this.stableSwap.state.targetAmpFactor.toString()), [math.ONE, math.ONE], // TODO: How to get the token decimal?
    new math.Fraction(this.stableSwap.state.fees.trade.numerator, this.stableSwap.state.fees.trade.denominator));
  }
...
class CremaAmm {
  ...
  preSwapA(amountIn) {
    ...
    let result = cremaSdk.calculateSwapA2B(this.ticks, this.poolState.currentSqrtPrice, this.poolState.fee, this.poolState.currentLiquity, amountIn);
  ...
  preSwapB(amountIn) {
    ...
    let result = cremaSdk.calculateSwapB2A(this.ticks, this.poolState.currentSqrtPrice, this.poolState.fee, this.poolState.currentLiquity, amountIn);
    let currentPriceA = this.poolState.currentSqrtPrice.pow(2);
    let currentPriceB = new Decimal__default["default"](1).div(currentPriceA);
...
const getCacheMintKey = marketInfos => {
  ...
  return `${(_marketInfos$ = marketInfos[0]) === null || _marketInfos$ === void 0 ? void 0 : _marketInfos$.inputMint}-${(_marketInfos$2 = marketInfos[0]) === null || _marketInfos$2 === void 0 ? void 0 : _marketInfos$2.outputMint}-${(_marketInfos$3 = marketInfos[1]) === null || _marketInfos$3 === void 0 ? void 0 : _marketInfos$3.outputMint}-${marketInfos.map(item => item.amm.id)}`;
};
...
async function createAndCloseWSOLAccount(connection, owner, amount) {
  ...
  result.instructions = [];
  const toAccount = await splToken.Token.getAssociatedTokenAddress(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, WRAPPED_SOL_MINT, owner, true);
  const info = await connection.getAccountInfo(toAccount);

  if (info === null) {
    result.instructions.push(splToken.Token.createAssociatedTokenAccountInstruction(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, WRAPPED_SOL_MINT, toAccount, owner, owner));
  ...
  result.cleanupInstructions = [splToken.Token.createCloseAccountInstruction(splToken.TOKEN_PROGRAM_ID, toAccount, owner, owner, [])];
  return {
    address: toAccount,
    ...result
  };
...
async function findOrCreateAssociatedAccountByMint(connection, payer, owner, mintAddress, unwrapSOL) {
  ...
  const toAccount = await splToken.Token.getAssociatedTokenAddress(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, mint, owner, true);
  const cleanupInstructions = [];
  const instructions = [];
  const info = await connection.getAccountInfo(toAccount);

  if (info === null) {
    instructions.push(splToken.Token.createAssociatedTokenAccountInstruction(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, mint, toAccount, owner, payer));
  } // We close it when wrapped SOL


  if (mint.equals(WRAPPED_SOL_MINT) && unwrapSOL) {
    cleanupInstructions.push(splToken.Token.createCloseAccountInstruction(splToken.TOKEN_PROGRAM_ID, toAccount, owner, owner, []));
  }
...
class Jupiter {
  ...
  shouldLoadSerumOpenOrders) {
    ...
    this.exchange = async ({
      ...
      if (!instructions) {
        const [ataInstructions, openOrdersInstructions] = await Promise.all([routeAtaInstructions(connection, routeInfo.marketInfos, owner.publicKey, _wrapUnwrapSOL), Promise.all(routeInfo.marketInfos.map(async ({
          amm
        }) => {
          if (amm instanceof SerumAmm || amm instanceof SplitTradeAmm) {
            if (!amm.market) return;
      ...
      const sourceInstruction = inputMint.equals(WRAPPED_SOL_MINT) && _wrapUnwrapSOL ? await createAndCloseWSOLAccount(connection, owner.publicKey, routeInfo.inAmount) : { ...getEmptyInstruction(),
        address: await splToken.Token.getAssociatedTokenAddress(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, inputMint, owner.publicKey, true)
  ...
  async executeInternal({
    ...
    try {
      ...
      var _setupInstructions$in;

      const hasOpenOrders = setupInstructions === null || setupInstructions === void 0 ? void 0 : setupInstructions.openOrders.some(oo => oo === null || oo === void 0 ? void 0 : oo.instructions.length);

      if (hasOpenOrders || setupInstructions !== null && setupInstructions !== void 0 && (_setupInstructions$in = setupInstructions.intermediate) !== null && _setupInstructions$in !== void 0 && _setupInstructions$in.instructions.length || setupInstructions !== null && setupInstructions !== void 0 && setupInstructions.destination.instructions.length) {
