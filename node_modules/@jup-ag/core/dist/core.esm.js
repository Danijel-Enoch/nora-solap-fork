...
class SaberAmm {
  constructor(stableSwap) {
    ...
    this.calculator = new Stable(TWO, JSBI.BigInt(this.stableSwap.state.targetAmpFactor.toString()), [ONE, ONE], // TODO: How to get the token decimal?
...
class CremaAmm {
  ...
  preSwapA(amountIn) {
    ...
    let result = calculateSwapA2B(this.ticks, this.poolState.currentSqrtPrice, this.poolState.fee, this.poolState.currentLiquity, amountIn);
  ...
  preSwapB(amountIn) {
    ...
    let result = calculateSwapB2A(this.ticks, this.poolState.currentSqrtPrice, this.poolState.fee, this.poolState.currentLiquity, amountIn);
    let currentPriceA = this.poolState.currentSqrtPrice.pow(2);
...
async function createAndCloseWSOLAccount(connection, owner, amount) {
  ...
  result.instructions = [];
  const toAccount = await Token$1.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, WRAPPED_SOL_MINT, owner, true);
  const info = await connection.getAccountInfo(toAccount);

  if (info === null) {
    result.instructions.push(Token$1.createAssociatedTokenAccountInstruction(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, WRAPPED_SOL_MINT, toAccount, owner, owner));
  ...
  result.cleanupInstructions = [Token$1.createCloseAccountInstruction(TOKEN_PROGRAM_ID, toAccount, owner, owner, [])];
  return {
    address: toAccount,
    ...result
  };
...
async function findOrCreateAssociatedAccountByMint(connection, payer, owner, mintAddress, unwrapSOL) {
  ...
  const toAccount = await Token$1.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, mint, owner, true);
  const cleanupInstructions = [];
  const instructions = [];
  const info = await connection.getAccountInfo(toAccount);

  if (info === null) {
    instructions.push(Token$1.createAssociatedTokenAccountInstruction(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, mint, toAccount, owner, payer));
  } // We close it when wrapped SOL


  if (mint.equals(WRAPPED_SOL_MINT) && unwrapSOL) {
    cleanupInstructions.push(Token$1.createCloseAccountInstruction(TOKEN_PROGRAM_ID, toAccount, owner, owner, []));
  }
  ...
  shouldLoadSerumOpenOrders) {
    ...
    this.exchange = async ({
      ...
      const sourceInstruction = inputMint.equals(WRAPPED_SOL_MINT) && _wrapUnwrapSOL ? await createAndCloseWSOLAccount(connection, owner.publicKey, routeInfo.inAmount) : { ...getEmptyInstruction(),
        address: await Token$1.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, inputMint, owner.publicKey, true)
