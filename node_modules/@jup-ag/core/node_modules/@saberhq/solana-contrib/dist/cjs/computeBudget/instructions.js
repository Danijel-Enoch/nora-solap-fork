...
const requestComputeUnitsInstruction = (units, additionalFee) => {
    const data = Buffer.alloc(layouts_1.RequestUnitsLayout.span);
    layouts_1.RequestUnitsLayout.encode({ instruction: 0, units, additionalFee }, data);
    return new web3_js_1.TransactionInstruction({
        data,
        keys: [],
        programId: index_1.COMPUTE_BUDGET_PROGRAM,
    });
};
...
const requestHeapFrameInstruction = (bytes) => {
    const data = Buffer.alloc(layouts_1.RequestHeapFrameLayout.span);
    layouts_1.RequestHeapFrameLayout.encode({ instruction: 1, bytes }, data);
    return new web3_js_1.TransactionInstruction({
        data,
        keys: [],
        programId: index_1.COMPUTE_BUDGET_PROGRAM,
